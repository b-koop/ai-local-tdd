...
function parseResult(output, phase) {
    const marker = output.match(/FORGE_PHASE_RESULT:(\{[^\n]+\})/);
    if (!marker)
        throw new Error(`Forge ${phase} worker did not return FORGE_PHASE_RESULT`);
    const parsed = JSON.parse(marker[1]);
    if (parsed.phase !== phase)
        throw new Error(`Forge worker returned ${parsed.phase}, expected ${phase}`);
    return parsed;
}
export async function runForgePhaseInSandbox(request) {
    ...
    try {
        ...
        if (String(before.stdout).trim() !== String(after.stdout).trim())
            throw new Error("sandbox HEAD changed unexpectedly");
        return result;
    }
    finally {
        if (sandbox) {
            try {
                await execFileAsync("sbx", ["rm", sandbox], { cwd: request.cwd, timeout: timeoutMs });
            }
            catch { }
        }
...
//# sourceMappingURL=forge-processor.js.map